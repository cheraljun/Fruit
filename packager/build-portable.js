...
async function buildModules() {
  ...
  // 先安装 shared 的依赖（backend 依赖它）
  log('  检查 shared 依赖...');
  const sharedDir = path.join(CONFIG.projectRoot, 'shared');
  const sharedNodeModules = path.join(sharedDir, 'node_modules');
  if (!fs.existsSync(sharedNodeModules)) {
    await runCommand('npm install', sharedDir, '  安装 shared 依赖');
  }
  logSuccess('shared 依赖已就绪');
...
async function copyAppFiles() {
  ...
  logSuccess('backend 复制完成（包含编译后的 shared）');
  
  // 复制 shared 配置文件（用于安装依赖）
  log('  复制 shared 配置...');
  const sharedSrc = path.join(CONFIG.projectRoot, 'shared');
  const sharedDest = path.join(appDir, 'shared');
  ...
  const sharedLockFile = path.join(sharedSrc, 'package-lock.json');
  if (existsSync(sharedLockFile)) {
    copyFile(sharedLockFile, path.join(sharedDest, 'package-lock.json'));
  } else {
    logWarning('shared/package-lock.json 不存在');
  }
  
  logSuccess('shared 配置复制完成');
  ...
  const terminalPlayerSrc = path.join(CONFIG.projectRoot, 'player-standalone', 'terminal', 'dist');
  const vnPlayerSrc = path.join(CONFIG.projectRoot, 'player-standalone', 'visual-novel', 'dist');
  
  if (fs.existsSync(terminalPlayerSrc) || fs.existsSync(vnPlayerSrc)) {
    log('  复制 player-standalone...');
    const playerDest = path.join(appDir, 'player-standalone');
    
    if (fs.existsSync(terminalPlayerSrc)) {
      copyDir(terminalPlayerSrc, path.join(playerDest, 'terminal', 'dist'));
      logSuccess('  - terminal 播放器复制完成');
    }
...
async function installProductionDeps() {
  ...
  const modules = [
    { dir: path.join(CONFIG.tempDir, 'app', 'shared'), name: 'shared' },
    { dir: path.join(CONFIG.tempDir, 'app', 'backend'), name: 'backend' },
  ];
  ...
  for (const module of modules) {
    ...
    const lockFile = path.join(module.dir, 'package-lock.json');
    const hasLockFile = existsSync(lockFile);
    
    if (hasLockFile) {
      log(`    使用 npm ci（有 lock 文件）`);
      await runCommand('npm ci --omit=dev --no-audit --no-fund', module.dir);
    } else {
      log(`    使用 npm install（无 lock 文件）`);
      await runCommand('npm install --omit=dev --no-audit --no-fund', module.dir);
    }
...
async function generateLauncher() {
  ...
  log('  生成 launcher.js...');
  copyFile(
    path.join(templatesDir, 'launcher.js'),
  ...
  log('  生成启动脚本...');
  copyFile(
    path.join(templatesDir, '双击启动.cmd'),
  ...
  log('  生成使用说明...');
  copyFile(
    path.join(templatesDir, '使用说明.txt'),
