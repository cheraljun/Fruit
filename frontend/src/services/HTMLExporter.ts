...
import type { Story } from '../../../shared/types/index';
...
export class HTMLExporter {
  ...
  async loadTemplates(): Promise<void> {
    try {
      // 加载Terminal模板
      const terminalRes = await fetch('/templates/terminal-player.html');
      if (terminalRes.ok) {
        this.templates.set('terminal', await terminalRes.text());
      }
      ...
      throw new Error('播放器模板未构建，请先运行：node build-standalone-players.js');
    }
  }
   ...
   * 导出故事为HTML
   */
  async exportToHTML(story: Story, mode: 'terminal' | 'visual-novel', customStyleCSS?: string): Promise<void> {
