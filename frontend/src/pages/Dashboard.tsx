...
import { DEFAULT_STORY_META, DEFAULT_START_NODE } from '../../../shared/constants/defaults.ts'
...
function Dashboard(): JSX.Element {
  ...
  function loadGuestStories(): void {
    try {
      const data = guestStorage.getAll()
      setStories(data)
    } catch (error) {
      console.error('加载游客故事失败:', error)
    } finally {
      setLoading(false)
    }
  }
  ...
  async function handleExport(id: string, e: React.MouseEvent): Promise<void> {
    e.stopPropagation()
  ...
  async function handleExportHTML(id: string, mode: 'terminal' | 'visual-novel', e: React.MouseEvent): Promise<void> {
    e.stopPropagation()
    ...
    try {
      ...
      const modeName = mode === 'terminal' ? '终端模式' : '视觉小说模式'
      notification.info(`正在导出${modeName}HTML，请稍候...`)
      
      // 获取当前激活的样式插件CSS
      let customStyleCSS = ''
      const compatibleWith = mode === 'terminal' ? 'terminal' : 'visual-novel'
      ...
      notification.success(`${modeName}HTML已导出`)
    } catch (error) {
  ...
  async function handleImport(): Promise<void> {
    ...
    input.onchange = async (e) => {
      ...
      try {
        ...
        if (username) {
          await api.drafts.save(newStory)
          notification.success('故事已导入')
          await loadStories()
        } else {
          guestStorage.save(newStory)
          notification.success('故事已导入（游客模式）')
          loadGuestStories()
        }
      } catch (error) {
  ...
  if (!username && !isGuestMode) {
    return <LoginCard onSuccess={handleLoginSuccess} onGuestMode={handleGuestMode} />
  }
    ...
    <div className="dashboard">
      {isGuestMode && !username && (
        <div className="guest-mode-notice">
          <span>当前为游客模式，数据仅保存在本浏览器标签页，关闭后将丢失。</span>
          <button onClick={() => setIsGuestMode(false)} className="notice-login-btn">
            登录保存作品
          </button>
        </div>
      )}
...
function LoginCard({ onSuccess, onGuestMode }: LoginCardProps): JSX.Element {
        ...
        <div className="login-card">
          ...
          {loginMethod === 'password' && (
            <form onSubmit={handlePasswordLogin} className="login-form">
              ...
              <button type="button" className="btn-guest" onClick={onGuestMode}>
                游客模式
              </button>
              
              <p className="guest-note">游客模式下数据仅保存在本浏览器标签页</p>
            </form>
          )}
