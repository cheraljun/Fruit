  ...
  private handleContentRender(data: any, _context: any): any {
    ...
    let processed = renderedHTML.replace(pattern, (_match: string, content: string) => {
      ...
      if (matchedChoice) {
        ...
        const linkHtml = `<a class="choice-embed-link" data-choice-id="${matchedChoice.id}" style="color:#5bc0de;cursor:pointer;text-decoration:underline;transition:all 0.2s;">${matchedChoice.text}</a>`;
        return linkHtml;
      }
